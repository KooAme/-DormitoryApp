...
function BusRequest(props) {
  ...
  const handleConfirm = date => {
    if (moment(date).isAfter(new Date())) {
      setDate(moment(date).format('YYYY년 MMMM Do dddd'));
      hideDatePicker();
    } else {
  ...
  return (
    <>
      ...
      <View
        ...
        <View style={box.containers}>
          <View style={box.place}>
            <Text style={font.title}>정류장</Text>
          </View>
          <View style={{borderWidth: 1}}>
            {direction == '복현 캠퍼스' ? (
              <Picker>
        ...
        <View style={box.containers}>
          <View style={box.place}>
            <Text style={font.title}>출발 시간</Text>
          </View>
          <View style={box.time}>
            <Text>06 : 00</Text>
            {/*이거 나중에 state 로 관리
